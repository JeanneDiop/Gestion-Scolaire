    ...
    public function rules(): array
    {
        ...
        if (auth()->check() && auth()->user()->role_nom === 'admin') {
            $rules = array_merge($rules, [
                ...
                'academies' => 'sometimes|required|boolean',
                'ressources' => 'sometimes|required|boolean',
                'rapports' => 'sometimes|required|boolean',
            ]);
        }
        ...
        if ($this->input('is_tuteur') == 0) {
            ...
            if (auth()->check() && auth()->user()->role_nom === 'admin') {
                $tuteurRules = array_merge($tuteurRules, [
                    ...
                    'tuteur.academies' => 'sometimes|required|boolean',
                    'tuteur.ressources' => 'sometimes|required|boolean',
                    'tuteur.rapports' => 'sometimes|required|boolean',
                ]);
            }

            $rules = array_merge($rules, $tuteurRules);
        }
